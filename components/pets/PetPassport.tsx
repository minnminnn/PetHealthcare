...
import {
  PawPrint, Shield, Chip, Weight, Calendar,
  Droplets, Heart, Download, Share2, QrCode
} from "lucide-react";
...
export function PetPassport({ pet, baseUrl = "https://petcare.vn" }: PetPassportProps) {
      ...
      >
        ...
        <div className="bg-gradient-medical px-8 py-6 flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <div className="text-white/90 text-xs font-mono mt-1">{pet.passportNumber}</div>
          </div>
        </div>
        ...
        <div className="p-8">
          <div className="flex gap-6 items-start">
            ...
            <div className="relative flex-shrink-0">
              ...
              {pet.isNeutered && (
                <div className="absolute -bottom-1 -right-1 w-7 h-7 bg-secondary-500 rounded-full flex items-center justify-center border-2 border-white" title="Đã triệt sản">
            ...
            <div className="flex-1 space-y-2">
              ...
              <div className="text-slate-500 font-medium">
                {SPECIES_EMOJI[pet.species]} {pet.breed ?? pet.species} · {pet.color ?? ""}
              </div>
              <div className="grid grid-cols-2 gap-3 mt-3">
                {[
                  { icon: Calendar, label: "Tuổi", value: getAge(pet.dateOfBirth) },
                  { icon: Weight, label: "Cân nặng", value: pet.weight ? `${pet.weight} kg` : "Chưa cập nhật" },
                  { icon: Droplets, label: "Nhóm máu", value: BLOOD_TYPE_DISPLAY[pet.bloodType] ?? pet.bloodType },
                  { icon: Heart, label: "Giới tính", value: pet.gender === "male" ? "Đực" : pet.gender === "female" ? "Cái" : "Không rõ" },
          ...
          {pet.microchipId && (
            <div className="mt-5 flex items-center gap-3 bg-slate-50 rounded-xl px-4 py-3 border border-slate-200">
              <Chip className="w-5 h-5 text-secondary-600" />
              <div>
              ...
              {pet.vaccinations.slice(0, 3).map((vax, i) => (
                <div key={i} className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-700 font-medium">{vax.vaccineName}</span>
                  </div>
                  <div className="flex items-center gap-3 text-slate-500">
                    <span>{new Date(vax.administeredAt).toLocaleDateString("vi-VN")}</span>
                    {vax.nextDueAt && (
                      <span className={`font-medium ${
                        new Date(vax.nextDueAt) < new Date() ? "text-red-500" : "text-green-600"
                      }`}>
                        Nhắc: {new Date(vax.nextDueAt).toLocaleDateString("vi-VN")}
                      </span>
                    )}
                  </div>
                </div>
          ...
          <div className="flex items-end justify-between">
            <div>
              ...
              <div className="font-semibold text-slate-800">{pet.owner?.name ?? "Không rõ"}</div>
            </div>
